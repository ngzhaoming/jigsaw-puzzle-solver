...
# Import the full image
canvas = cv2.imread('resources/forest/forest_cropped2.jpg')

# Import the pieced
piece = cv2.imread('resources/forest/forest_piece2.jpg')

# Pre-process the piece
def identify_contour(piece, threshold_low=120, threshold_high=255):
    """Identify the contour around the piece"""
    piece = cv2.cvtColor(piece, cv2.COLOR_BGR2GRAY) # better in grayscale
    ret, thresh = cv2.threshold(piece, threshold_low, threshold_high, 0) # Binarize image
    cv2.imshow("Thresh", thresh)
    cv2.waitKey(0)
    contours, heirarchy = cv2.findContours(thresh, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    contour_sorted = np.argsort(map(cv2.contourArea, contours))

    if len(contour_sorted) > 2:
...
# find the keypoints and descriptors (surrounding pixels) with SIFT
